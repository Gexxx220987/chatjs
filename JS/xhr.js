...
let userLoggedId; // ID
let currentRoom = ''; // откл при переходе в новую комнату

// рандом номер для WS для возможности одновременного открытия
let sn =  'sn' + Math.floor(Math.random() * 10000 + 1);
console.log("Сформирован рандом номер для WebSoket:", sn);


const socket = new WebSocket('ws://localhost:8000/ws/instructions/' + sn); // комадирский канал
const chatSocket = new WebSocket('ws://localhost:8000/ws/chat/' + sn); // сообщения чата
socket.onclose = async (event) => {
    console.error(event);
    window.alert('что то пошло не так - проверь связь с сервером!');
    console.log('что то пошло не так - проверь связь с сервером!');
};
...
async function editAvatar(userId) {
    ...
    if (fileField.files[0]) {
        ...
        try {
            ...
            console.error('Что то не то', error);
        }
        viewUserCard(userId);
    } else {
...
function changeUserName(userId) {
    ...
    console.log('изменить имя на:', name);
};
...
function printChat(data) {
    ...
    <input type="text" id="input_message" name="input_message" size="25" placeholder="Введите сообщение"><br>
